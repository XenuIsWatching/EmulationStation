...
MarqueeGameListView::MarqueeGameListView(Window* window, FileData* root) :
	ISimpleGameListView(window, root),
	mMarqueeList(window), mMarquee(window),
	mThumbnail(window),
	mImage(window),
	mVideo(nullptr),
	mVideoPlaying(false),
...
// Create the correct type of video window
	...
	// Marquee list on the left side
	mMarqueeList.setPosition(0, mSize.y() * 0.1f);
	mMarqueeList.setSize(mSize.x() * 0.48f, mSize.y() * 0.8f);
	mMarqueeList.setDefaultZIndex(20);
	mMarqueeList.setCursorChangedCallback([&](const CursorState& /*state*/) { updateInfoPanel(); });
	addChild(&mMarqueeList);
	...
	mRomNameContainer.setAutoScroll(true, true);
	...
	mDescContainer.setPosition(mSize.x() * 0.50f + mSize.x() * padding, mSize.y() * 0.65f);
	...
	mVideo->setPosition(mSize.x() * 0.75f, mSize.y() * 0.4f);
	mVideo->setSize(mSize.x() * 0.45f, mSize.y() * 0.45f);
	mVideo->setDefaultZIndex(30);
	mVideo->setVisible(false);
	addChild(mVideo);

	// Marquee image (detail panel, not the list)
...
MarqueeGameListView::~MarqueeGameListView()
{
	delete mVideo;
}

FileData* MarqueeGameListView::getCursor()
{
	return mMarqueeList.getSelected();
}

void MarqueeGameListView::setCursor(FileData* file, bool refreshListCursorPos)
{
	if(!mMarqueeList.setCursor(file) && (!file->isPlaceHolder()))
	{
		populateList(file->getParent()->getChildrenListToDisplay());
		mMarqueeList.setCursor(file);
	}
}

std::string MarqueeGameListView::getQuickSystemSelectRightButton()
{
	return "righttrigger";
}

std::string MarqueeGameListView::getQuickSystemSelectLeftButton()
{
	return "lefttrigger";
}

bool MarqueeGameListView::input(InputConfig* config, Input input)
{
	return ISimpleGameListView::input(config, input);
}

void MarqueeGameListView::populateList(const std::vector<FileData*>& files)
{
	mMarqueeList.clear();
	...
	if (files.size() > 0)
	{
		...
		{
			mMarqueeList.add((*it)->getName(), (*it)->getMarqueePath(), *it);
		}
	}
	else
	{
...
void MarqueeGameListView::onThemeChanged(const std::shared_ptr<ThemeData>& theme)
{
	ISimpleGameListView::onThemeChanged(theme);

	using namespace ThemeFlags;

	mMarqueeList.applyTheme(theme, getName(), "gamelist", ALL);
	...
	// Repopulate list in case theme changed properties. Preserve selection.
	FileData* file = (mMarqueeList.size() > 0) ? mMarqueeList.getSelected() : nullptr;
...
void MarqueeGameListView::initMDLabels()
{
	...
	Vector3f start(mSize.x() * 0.50f + mSize.x() * 0.01f, mSize.y() * 0.625f, 0.0f);
...
void MarqueeGameListView::initMDValues()
{
	std::vector<TextComponent*> labels = getMDLabels();
	std::vector<GuiComponent*> values = getMDValues();
...
void MarqueeGameListView::updateInfoPanel()
{
	FileData* file = (mMarqueeList.size() == 0 || mMarqueeList.isScrolling()) ? NULL : mMarqueeList.getSelected();
...
void MarqueeGameListView::addPlaceholder()
{
	...
	mMarqueeList.add(placeholder->getName(), "", placeholder);
}

void MarqueeGameListView::launch(FileData* game)
{
...
void MarqueeGameListView::remove(FileData *game, bool deleteFile, bool refreshView)
{
	if (deleteFile)
...
std::vector<TextComponent*> MarqueeGameListView::getMDLabels()
{
...
std::vector<GuiComponent*> MarqueeGameListView::getMDValues()
{
...
std::vector<HelpPrompt> MarqueeGameListView::getHelpPrompts()
{
...
void MarqueeGameListView::update(int deltaTime)
{
	ISimpleGameListView::update(deltaTime);
	mVideo->update(deltaTime);
}

void MarqueeGameListView::onShow()
{
	GuiComponent::onShow();
	updateInfoPanel();
}

void MarqueeGameListView::onFocusLost()
{
	mDescContainer.reset();
}
